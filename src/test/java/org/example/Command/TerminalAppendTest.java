...
class TerminalAppendTest {
    ...
    void Append_pwd() {
        ...
        String[] args2 ={">>","student"};
        terminal.pwd("pwd",args2);
        String[] file ={"student"};
        terminal.cat(file);
        String answer = (current.toString().trim())+(current.toString().trim());
        assertEquals(answer,outputStream.toString().trim());

    }

    @Test
    void Append_pwd_Logical() {
        String[] args ={">","student"};
        terminal.pwd("pwd -L",args);  //
        String[] args2 ={">>","student"};
        terminal.pwd("pwd -L",args2);
        String[] file ={"student"};
        terminal.cat(file);
        String answer = (current.toString().trim())+(current.toString().trim());
        assertEquals(answer,outputStream.toString().trim());
